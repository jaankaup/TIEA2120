...
/* TASO 1 funktiokutsut.*/
console.log("TASO 1 funktiokutsut.");

/* Lisataan mallijoukkue. */
lisaaJoukkue({"nimi": "Mallijoukkue",
...
/* Tulostetaan rastien koodit ; merkilla erotettuna. */
tulostaIntAlkuinen(data);


/* TASO 3 funktiokutsut. */
console.log("TASO 3 funktiokutsut.");
poistaJoukkue(data, "Vara 1");
poistaJoukkue(data, "Vara 2");
poistaJoukkue(data, "Vapaat");

/* Tulostetaan joukkueet. */
console.log("Tulostaa joukkueet datan, josta on poistettu joukkueet Vara 1, Vara 2 seka Vapaat.");
tulostaJoukkue(data);

/* Tulostetaan joukkueet pistemaarittain suurimmasta pienimpaan. */
console.log("Tulostaa joukkueiden nimet ja pistemaarat laskevassa jarjestyksessa pisteiden mukaan.");
tulostaJoukkueetPojot(data);

/* Tulostetaan taso 5. */
console.log("Tahan tulee tason 5 tulostus:");
tehtava5(data);


/************* FUNKTIOT ***************/

/* funktio, joka tulostaa parametrina x tuodun kisadatan kaikkien joukkueiden nimet. */
function tulostaJoukkue(x) {
  let sarjat = x["sarjat"];
  for (let i=0; i<sarjat.length ; i++)
  {
    let joukkueet = sarjat[i].joukkueet;
    for (let j=0; j<joukkueet.length; j++)
    {
       console.log(joukkueet[j].nimi);
    }
  }
}
...
/* 
 * Funktion parametrit:
 *   sarjan_nimi: merkkijono
 *   joukkue: joukkue data
 *   kisa_data: TIEA2120 kurssin kisa data
 *
 * Funktio, joka lisaa joukkueen sellaiseen sarjaan, joka loytyy kisa_datasta.
 */
function lisaaJoukkue(joukkue, sarjan_nimi, kisa_data) {
  let sarjat = kisa_data["sarjat"];
  for (let i=0; i<sarjat.length; i++)
  {
    if (sarjat[i].nimi == sarjan_nimi)
    {
      (sarjat[i].joukkueet).push(joukkue);
    }
  }
}

/* Funktio, joka tulostaa sellaiset rastit, joiden koodi alkaa kokonaisluvulla. */
function tulostaIntAlkuinen(malliData)
{
  let rastit = malliData["rastit"]; 
  let koodit = [];
  for (let i=0; i<rastit.length; i++)
  {
    let k = rastit[i].koodi;
    /* Jos matsaa siten, etta merkkijonon ensimmainen merkki on numero, niin... */
    if (k.match(/^\d/))
    {
      /* Lisätään löydetty koodi koodit listan. */
      koodit.push(k);
    }
  }
  /* Tulostetaan koodit ; merkilla erotettuna. */
  console.log(koodit.join(';'));
}

/* Funktio, joka poistaa joukkueen nimen perusteella. */
function poistaJoukkue(malliData, nimi) 
{
  let sarjat = malliData["sarjat"];
  for (let i=0; i<sarjat.length; i++)
  {
    let joukkueet = sarjat[i].joukkueet;
    for (let j=0; j<joukkueet.length; j++)
    {
      if (joukkueet[j].nimi == nimi)
      {
        /* poistetaan taulukosta joukkue. */
        joukkueet.splice(j,1);
      }
    }
  }
}

/* Funktio, joka laskee joukkueiden pisteet yhteen ja tulostaa ne pisteiden mukaan laskevassa jarjestyksessa. */
function tulostaJoukkueetPojot(malliData)
{
  /* Luodaan rastit datasta uusi versio {id, piste}, missa id on rastin id ja piste on koodista konvertoitu pistemaara. */
  let r = malliData["rastit"].map(x => { let k=0; if (x["koodi"].match(/^\d/)) k = parseInt(x["koodi"][0],10); return {id: (x.id).toString(), piste: k};});
  ...
  {
    /* Etsitaan ne tuvat, jotka matsaavat joukkueen id:n kanssa ja luodaan lista rastien id:sta muutettuna stringeiksi. */
    let t = (malliData["tupa"].filter(x => x["joukkue"] === joukkue.id)).map(x => (x.rasti).toString());
    /* Poistetaan joukkueiden kaymien rastien monikerrat. */
    let rastiSetti = Array.from(new Set(t));
    /* Lasketaan joukkueen rastien pisteet yhteen. */
    let pisteet = 0;
    for (let rasti of rastiSetti)
    {
      for (let rPiste of r)
      {
        /* Lasketaan kunkin rastin piste yhteen piste muuttujaan. */
        if (rasti === rPiste.id)
	{
          pisteet += rPiste.piste;
	  break;
	}
      }
    }
    joukkueData.push({joukkueNimi: joukkue.nimi, pisteet: pisteet});
  }
  /* Tulostaa joukkueen nimen ja sen saaman pistemaaran laskevassa jarjestyksessa. */
  for (let x of joukkueData.sort((a,b) => b.pisteet - a.pisteet))
  {
    console.log(x["joukkueNimi"] + " (" + x["pisteet"] + " p)");
  }
}
